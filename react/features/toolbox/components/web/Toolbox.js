...
import { enableCinemaView } from '../../../base/conference';
...
import SpaceDeck from './SpaceDeck';
import { toggleDocument } from '../../../../features/etherpad/actions';
...
class Toolbox extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this._onToolbarToggleCalendar = this._onToolbarToggleCalendar.bind(this);
    ...
    _doToggleSetting() {
        this.props.dispatch(openSettingsDialog(SETTINGS_TABS.MORE));
    }
    ...
    setSpaceDeck = (spaceDeck) => {
        this.setState({ spaceDeck })
    }
    ...
    */
    ...
    _onToolbarToggleCalendar() {
        // sendAnalytics(createToolbarEvent('calender'));
        sendAnalytics(createCalendarClickedEvent('moderator'));


        this._doToggleSetting();
    }
    ...
    _toggleCinemaView = (val) => {
        this.props.dispatch(enableCinemaView(val))
    }
    ...
    _renderDesktopSharingButton(isInOverflowMenu = false) {
        ...
        return (
            ...
            <ToolbarButton
                ...
                icon={IconShareDesktop}
                onClick={this._onToolbarToggleScreenshare}
                title={_screensharing ? 'Stop ' : 'Share ' + ' Screen'}
    ...
    _renderOverflowMenuContent() {
        ...
        //             text={_sharingVideo ? 'Cinema View' : 'Exit Cinema'}
        //             onClick={this._toggleCinemaView}
        ...
        return [
            ...
            //     icon={IconFullScreen}
            //     key='theater'
            //     onClick={() => this._toggleCinemaView(true)}
            //     text={'View theater mode'} />,
            // <OverflowMenuItem
            //     accessibilityLabel={t('toolbar.accessibilityLabel.sharedvideo')}
            //     icon={IconFullScreen}
            //     key='default'
            //     onClick={() => this._toggleCinemaView(false)}
            //     text={'View default screen'} />,
            ...
            <OverflowMenuItem
                ...
                text={'Collabore on a Google doc'} />,
            <OverflowMenuItem
                accessibilityLabel={t('toolbar.accessibilityLabel.sharedvideo')}
                icon={IconFullScreen}
                key='space'
                onClick={() => this.setSpaceDeck(true)}
                text={'Collabore on a whiteboard'} />,
    ...
    _renderToolboxContent() {
        ...
        if (this._shouldShowButton('settings')) {
            buttonsLeft.push('settings');
        }
        ...
        return (
            <div className='toolbox-content'>
                <div className='button-group-left'>
                    ...
                    {buttonsLeft.indexOf('settings') !== -1
                        && <ToolbarButton
                            accessibilityLabel={t('toolbar.Settings')}
                            icon={IconSettings}
                            onClick={this._onToolbarToggleCalendar}
                ...
                visible = { this._shouldShowButton('settings') } />*/}
                    {/* buttonsLeft.indexOf('settings') !== -1
                        && <div className = 'toolbox-setting-button'>
                        <SettingsButton
                        key = 'settings'
                        showLabel = { true }
                        visible = { this._shouldShowButton('settings') } />
        ...
        tooltip = { t('toolbar.raiseHand') } /> */}
                ...
                && <SecurityDialogButton customClass = 'security-toolbar-button' /> */}
                    ...
                    {this.props._screensharing == true && 
                        <SharedDocumentButton
                          key='etherpad'
                          showLabel={true} />}
            ...
            />*/}
                    ...
                    />
                    <SpaceDeck
                        show={this.state.spaceDeck}
                        onHide={() => this.setSpaceDeck(false)}
                    />
                </div>
            </div>);
    }
...
function _mapStateToProps(state) {
    const { conference, locked, cinema } = state['features/base/conference'];
